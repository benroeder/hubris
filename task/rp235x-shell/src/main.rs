...
/// Signed 16-bit sine table generated by build.rs, used by `wavgen` to
/// synthesize PCM. 256 entries (power of two) so a DDS indexes it by a shift.
#[cfg(feature = "fat")]
mod wavgen_lut {
    include!(concat!(env!("OUT_DIR"), "/sine_i16_lut.rs"));
}
...
const HELP_PWM: &[u8] = b"\
  ...
  audiosel [secs]       pick jack source with buttons (BT1=mic, other=sine)\r\n\
...
#[cfg(all(feature = "fat", feature = "pwm"))]
const HELP_MIX: &[u8] =
    b"  mix <name>            WAV/MP3/FLAC + 660Hz sine mixer (buttons crossfade)\r\n";
#[cfg(feature = "datalog")]
...
impl Shell {
    fn dispatch(&mut self, line: &str) {
        ...
        match cmd {
            ...
            #[cfg(feature = "fat")]
            "wavgen" => {
                self.cmd_wavgen(words.next(), words.next(), words.next())
            }
            #[cfg(feature = "fat")]
    ...
    fn cmd_led(&mut self, verb: Option<&str>, arg: Option<&str>) {
        // `arg` is only consumed by the pwm `dim` branch.
        #[cfg(not(feature = "pwm"))]
        let _ = arg;
        #[cfg(not(feature = "pwm"))]
        if verb == Some("dim") {
            self.out.put(b"led dim needs the pwm driver (not in this build)\r\n");
            return;
        }
        #[cfg(feature = "pwm")]
        if verb == Some("dim") {
    ...
    /// `wavgen <name> [secs] [hz]`: synthesize a canonical 16-bit mono PCM WAV
    /// (44-byte RIFF/fmt/data header + DDS sine) and write it to the SD root.
    /// Gives us a real audio file to stream without shuffling the card to a
    /// reader. Fixed 22050 Hz sample rate; secs default 3 (max 30), hz default
    /// 440. The player reads the rate back out of the header.
    #[cfg(feature = "fat")]
    fn cmd_wavgen(
        &mut self,
        name: Option<&str>,
        ...
        let Some(name) = name else {
            self.out.put(b"usage: wavgen <name> [secs] [hz]\r\n");
            return;
        };
        let secs = secs
            .and_then(|s| s.parse::<u32>().ok())
        ...
        let hz = hz
            ...
            .unwrap_or(440)
            .clamp(20, 10_000);
        let total_samples = RATE * secs;
        let data_bytes = total_samples * 2; // 16-bit mono
        // DDS: phase step per sample for `hz` at RATE, in a 32-bit accumulator.
        let phase_inc =
            ((hz as u64) << 32).checked_div(RATE as u64).unwrap_or(0) as u32;

        // Canonical 44-byte WAV header (PCM, mono, 16-bit).
        ...
        hdr[34..36].copy_from_slice(&16u16.to_le_bytes()); // bits/sample
        hdr[36..40].copy_from_slice(b"data");
        hdr[40..44].copy_from_slice(&data_bytes.to_le_bytes());

        self.with_root(b"wavgen", |sh, root| {
            let file = match root
                .open_file_in_dir(name, Mode::ReadWriteCreateOrTruncate)
            {
                Ok(f) => f,
                Err(_) => {
                    sh.out.put(b"wavgen: open file failed\r\n");
                    return;
                }
            };
            if file.write(&hdr).is_err() {
                sh.out.put(b"wavgen: header write failed\r\n");
                let _ = file.close();
                return;
            }
            // Stream the PCM in 512-byte chunks (256 samples) so we never hold
            // the whole track in RAM.
            let lut_mask = wavgen_lut::SINE_I16_LUT.len() as u32 - 1;
            let mut phase: u32 = 0;
            let mut buf = [0u8; 512];
            let mut done: u32 = 0;
            while done < total_samples {
                let n = core::cmp::min(256, total_samples - done) as usize;
                for slot in buf[..n * 2].chunks_exact_mut(2) {
                    let idx = ((phase >> 24) & lut_mask) as usize;
                    let s = wavgen_lut::SINE_I16_LUT[idx].to_le_bytes();
                    slot[0] = s[0];
                    slot[1] = s[1];
                    phase = phase.wrapping_add(phase_inc);
                }
                if file.write(&buf[..n * 2]).is_err() {
                    sh.out.put(b"wavgen: data write failed\r\n");
                    let _ = file.close();
                    return;
                }
                done += n as u32;
            ...
            if file.close().is_err() {
                sh.out.put(b"wavgen: flush/close error\r\n");
                return;
            }
            sh.out.put(b"wavgen ok: ");
            sh.out.put_u32(44 + data_bytes);
            sh.out.put(b" bytes (");
            sh.out.put_u32(RATE);
            sh.out.put(b" Hz, ");
            sh.out.put_u32(hz);
            sh.out.put(b" Hz tone)\r\n");
        });
    }
    ...
    /// meanwhile). Prints the detected sample rate on success. UNPROVEN on
    /// hardware.
    #[cfg(feature = "fat")]
    fn cmd_play(&mut self, name: Option<&str>) {
        let Some(name) = name else {
            self.out.put(b"usage: play <name>\r\n");
            return;
        };
        if name.len() > 12 {
        ...
        #[cfg(all(not(feature = "i2s"), not(feature = "pwm")))]
        self.out.put(b"play: no audio output in this build\r\n");
        #[cfg(all(not(feature = "i2s"), feature = "pwm"))]
        {
