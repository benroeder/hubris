...
| Self-reboot | `reboot` shell cmd: direct watchdog/PSM writes (ACCESSCTRL SU grants from startup) | uptime counter resets + fresh boot greeting. **BOOTSEL reboot parked**: calling any ROM *function* unprivileged faults (RCP-canary hypothesis; CPACR cp7 insufficient) -- needs the debug probe |
