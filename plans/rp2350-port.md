...
| Flash stage 1 | `lib/rp235x-romapi` (ROM table walk) + `drv/rp235x-flash{,-api}`: reads + ROM lookup; `rom`/`flash read` shell cmds | `rom FO/IF/EX/RE` return real ROM addrs; `flash read 160` dumps the image's own IMAGE_DEF; short reads at device tail. **Two MPU landmines found + documented**: PMSAv8 no-overlap (whole-flash reads use the 0x14000000 uncached mirror) and the per-task null region 0x0..0x20 (ROM header unreachable → walk the table via the 0x7df6 RISC-V pointer). Stage 2 (erase/program) needs RAM-resident kernel + ACCESSCTRL QMI grant |
| ADC + PWM | `drv/rp235x-adc{,-api}` (one-shot, ch4 = die temp; clk_adc 48 MHz) + `drv/rp235x-pwm{,-api}` (1 kHz duty) + gpio `set_function` | `temp` = 24.7 -> 26.6 C under load (correct polarity); `led dim 2` vs `100` visually distinct |
| GPIO pulls + E9 | `set_pull` op + shell `gpio pull`; E9 input-latch reproduced live (pull-down read 1 after a pull-up) and fixed via IE-cycle | floating GP2: up -> 1, down -> 0 (was 1 pre-fix), up -> 1 |
| RAM boot (default) | IMAGE_DEF LOAD_MAP: ROM copies the whole image to SRAM at boot; **XIP hazard eliminated**. Datasheet trap found via probe+bootrom source: absolute LOAD_MAP 3rd word = RUNTIME end, not storage end | ROM-copied vector table at 0x20000000; full system verified from RAM |
| Flash stage 2 | `erase`/`program` ops via QMI direct mode (W25Q commands) from the unprivileged flash server; shell `flash erase/write` | deadbeef programmed + read back + erased to FF on scratch sector, system live throughout -- **Hubris rewrites its own flash at runtime** |
| SWD debugging | debugprobe on a spare board; probe-rs attach, live memory reads, SWD flashing (2s) | root-caused the RAM-boot rejection via boot-RAM diagnostics in minutes |
| USB self-update | shell `update` + host `update-over-usb.py`: image over CDC, page-paced, CRC-verified, reboot to apply. Found+fixed: watchdog PAUSE_DBG wedge (reboot under debugger), flash-chip volatile state after direct mode | double round-trip v3->v4->v3, 3.6 s/58 KiB transfer, autonomous reboot, back in ~1 s |
| A/B update | 2-partition table (picotool) + versioned IMAGE_DEFs; ROM boots higher version; `update` writes to the inactive slot via physical (no-translate) reads matching QMI writes | arbitration proven both ways; self-update targets the stale slot (`target 0x42000`), leaves the running slot intact |
| Self-reboot | `reboot` shell cmd: direct watchdog/PSM writes (ACCESSCTRL SU grants from startup) | uptime counter resets + fresh boot greeting |
| BOOTSEL reboot | ROM calls proven privilege-gated by experiment (privileged flash_op OK, task call faults). `reboot bootsel` = two-hop: scratch0 marker + watchdog reset, privileged boot path calls ROM reboot | BOOTSEL device in ~1 s; full hands-free reflash cycle completed -- **the manual BOOTSEL dance is over** |
